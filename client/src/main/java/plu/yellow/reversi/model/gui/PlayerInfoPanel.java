...
public class PlayerInfoPanel extends JPanel {
    ...
    /**
     * Sets a player's scoer
     * @param player The player's score to be updated
     * @param board The current gameboard
     */
    public void setScore( int player, Board board ) {
        if( player == 1 ) {
            player1Score.setScoreValue(board.score(player));
        } else if (player == 2 ) {
            player2Score.setScoreValue(board.score(player));
        } else {
            throw new IllegalArgumentException("Invalid player: " + player);
        }
    }
    ...
    public ScoreIcon getScoreIcon(int plr){
        if(plr == 1)
            return player1Score;
        else
            return player2Score;
